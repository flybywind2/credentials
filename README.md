...
P0/P1 애플리케이션 기능은 구현되어 있으며, 외부 인프라가 필요한 실제 AD bind/SAML assertion 검증, 191개 파트/5,000건 성능 검증, Docker 실제 기동 검증은 운영 환경에서 별도 확인해야 합니다. 최신 상태는 [tasks.md](tasks.md)를 기준으로 합니다.
...
| --- | --- |
| `/api/health` | 서버 상태 확인 |
| `/api/auth/login` | 사번 기반 mock 로그인 API |
...
- 로그인 화면과 사번 기반 mock SSO 흐름은 구현되어 있습니다.
- `SSO_MODE=ldap` 또는 `saml` 설정값은 받지만, 실제 AD bind/SAML assertion 검증은 사내 IdP 정보와 인증 라이브러리 확정 후 `backend/services/sso.py`에서 교체해야 합니다.
- 개발 mock 모드의 `/api/auth/me`는 `X-Employee-Id` 헤더를 신뢰합니다. 운영에서는 reverse proxy가 안전하게 주입하거나 backend가 직접 token/assertion을 검증해야 합니다.
- 운영에서 `mock-token-*`은 사용하지 않습니다.
...
`SSO_MODE=ldap|saml`이면 SSO 관련 값이 필수입니다. `SMTP_MODE=smtp`이면 SMTP 관련 값이 필수입니다. 누락 시 앱 시작 단계에서 명확한 오류를 발생시킵니다.
...
- 실제 사내 AD bind 또는 SAML assertion 검증
...
- `.env`, `.env.private-cloud`, DB 비밀번호, SSO secret, SMTP 비밀번호는 커밋하지 않습니다.
- `.env.example`만 템플릿으로 커밋합니다.
- 운영 DB 계정은 앱 DB에 필요한 권한만 부여합니다.
- 실제 SSO 전환 전까지 `X-Employee-Id` 헤더 신뢰 구조를 외부에 노출하지 않습니다.
...
`SSO_MODE=ldap` 또는 `saml`인데 SSO 값이 비어 있으면 시작 중 오류가 납니다.
