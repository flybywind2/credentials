...
| `/api/auth/me` | 현재 사용자 조회. mock은 개발 header/cookie fallback, broker는 broker 내부 header 사용 |
...
- 개발 mock 모드에서만 `/api/auth/me`가 `X-Employee-Id` fallback을 허용합니다. 운영 broker 모드는 `SSO_BROKER_EMPLOYEE_HEADER`가 필요합니다.
- 운영 전에는 사내 SSO broker 헤더명, `deptname` 조직 매핑, 방화벽/DNS를 실제 환경에서 검증해야 합니다.
...
`SSO_MODE=broker`이면 broker employee header가 필요합니다. `MAIL_MODE=mail_api`이면 `MAIL_API_BASE_URL`이 필수입니다. 누락 시 앱 시작 단계에서 명확한 오류를 발생시킵니다.
...
- 운영에서는 `SSO_MODE=broker`를 사용하고, mock 모드의 `X-Employee-Id` fallback을 외부에 노출하지 않습니다. Broker 모드는 외부 요청의 `X-Broker-*` 헤더를 proxy에서 제거한 뒤 인증 후 재주입해야 합니다.
...
`SSO_MODE=broker`인데 broker employee header가 비어 있거나, `MAIL_MODE=mail_api`인데 `MAIL_API_BASE_URL`이 비어 있으면 시작 중 오류가 납니다.
