...
- 메일 서비스 인터페이스(SMTP/사내 API), HTML 알림 템플릿, 발송 실패 로깅
- 일회성 취합 종료/재오픈, 최종 Export 이력, 최소 감사 로그, 메일 실패 관리자 확인
- Docker 실행 파일과 사내 프라이빗 클라우드용 compose 파일
- Kubernetes 기본 배포 매니페스트

P0/P1 애플리케이션 기능과 broker/LDAP/SAML SSO 연동 코드는 구현되어 있으며, 외부 인프라가 필요한 사내 IdP 접속 검증, 191개 파트/5,000건 성능 검증, Docker 실제 기동 검증은 운영 환경에서 별도 확인해야 합니다. 최신 상태는 [tasks.md](tasks.md)를 기준으로 합니다.
...
| `/api/auth/me` | 현재 사용자 조회. mock은 개발 header fallback, broker는 broker 내부 header, LDAP/SAML은 bearer token 사용 |
...
- `SSO_MODE=broker`는 사내 SSO broker/reverse proxy가 주입한 내부 사용자 헤더를 신뢰합니다.
...
- 개발 mock 모드에서만 `/api/auth/me`가 `X-Employee-Id` fallback을 허용합니다. 운영 broker 모드는 `SSO_BROKER_EMPLOYEE_HEADER`, LDAP/SAML 모드는 bearer token이 필요합니다.
- 운영 전에는 사내 IdP URL, 인증서, bind DN, 사용자 속성명, 방화벽/DNS를 실제 환경에서 검증해야 합니다.

## 사내 LLM용 SSO/메일 연동 가이드

성능이 낮은 내부 LLM이나 자동화 에이전트가 SSO 또는 메일 연동 코드를 수정해야 할 때는 [docs/sso-email-llm-guide.md](docs/sso-email-llm-guide.md)를 먼저 읽게 합니다. 이 문서는 broker/LDAP/SAML 선택 기준, `/api/auth/me`와 bearer token 사용 방식, 사내 `POST /send_mail` payload 경계를 코드 위치와 함께 정리합니다.
...
| `SSO_MODE` | `mock` | `mock`, `broker`, `ldap`, `saml` |
...
| `SMTP_MODE` | `disabled` | `disabled`, `smtp`, `mail_api` |
...
`SSO_MODE=broker`이면 broker employee header가 필요하고, `SSO_MODE=ldap|saml`이면 각 모드의 provider URL, token secret, LDAP/SAML 세부 값이 필수입니다. `SMTP_MODE=smtp`이면 SMTP 관련 값이 필요하고, `SMTP_MODE=mail_api`이면 `MAIL_API_BASE_URL`이 필수입니다. 누락 시 앱 시작 단계에서 명확한 오류를 발생시킵니다.

`SMTP_MODE=mail_api`는 사내 메일 라우터 `POST /send_mail`로 JSON payload `{ "recipients": ["id@samsung.com"], "title": "제목", "content": "<html>" }`를 보냅니다. `docSecuType`, `contentType`, `recipientType` 조립은 메일 라우터가 담당합니다.
...
- `.env`, `.env.private-cloud`, DB 비밀번호, SSO token/client secret, SAML 인증서, SMTP 비밀번호, 사내 메일 API 식별자는 커밋하지 않습니다.
- `.env.example`만 템플릿으로 커밋합니다.
- 운영 DB 계정은 앱 DB에 필요한 권한만 부여합니다.
- 운영에서는 `SSO_MODE=broker|ldap|saml`을 사용하고, mock 모드의 `X-Employee-Id` fallback을 외부에 노출하지 않습니다. Broker 모드는 외부 요청의 `X-Broker-*` 헤더를 proxy에서 제거한 뒤 인증 후 재주입해야 합니다.
...
`SSO_MODE=broker`인데 broker employee header가 비어 있거나, `SSO_MODE=ldap|saml`인데 provider URL, token secret, LDAP/SAML 필수 값이 비어 있으면 시작 중 오류가 납니다.
