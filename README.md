...
- 업무 CRUD, 상세 입력 모달, Excel 붙여넣기/Import 미리보기 저장 흐름
...
- 메일 서비스 인터페이스(사내 mail API), HTML 알림 템플릿, 발송 실패 로깅
- 일회성 취합 종료/재오픈, 최종 Export 이력, 최소 감사 로그, 메일 실패 관리자 확인
- Docker 실행 파일과 사내 프라이빗 클라우드용 compose 파일
- Kubernetes 기본 배포 매니페스트

P0/P1 애플리케이션 기능과 mock/broker SSO 연동 코드는 구현되어 있으며, 외부 인프라가 필요한 사내 SSO broker 접속 검증, 191개 파트/5,000건 성능 검증, Docker 실제 기동 검증은 운영 환경에서 별도 확인해야 합니다. 최신 상태는 [tasks.md](tasks.md)를 기준으로 합니다.
...
개발 기본값은 `.env.example`과 동일하게 SQLite, mock SSO, disabled mail delivery입니다.
...
| --- | --- |
| `/api/health` | 서버 상태 확인 |
| `/api/auth/login` | mock 로그인 및 bearer token 발급 |
| `/api/auth/sso-config` | 프론트엔드가 사용할 SSO mode, `BROKER_URL`, `SERVICE_URL` 조회 |
| `/api/auth/broker/session` | broker callback query(`loginid`, `deptname`, `username`)를 앱 세션으로 교환 |
| `/api/auth/me` | 현재 사용자 조회. mock은 개발 header/cookie fallback, broker는 broker callback 세션 또는 내부 header 사용 |
...
| `/api/admin/collection/status` | 일회성 취합 종료/재오픈 상태 조회 및 변경 |
| `/api/admin/audit-logs` | 관리자 감사 로그 조회 |
...
- `SSO_MODE=broker`는 `BROKER_URL`로 이동해 사내 SSO 인증을 수행하고, broker가 `SERVICE_URL/?loginid=...&deptname=...&username=...`으로 돌려준 값을 앱 세션으로 교환합니다. 내부 header 주입 방식도 보조로 지원합니다.
- 인증 성공 후 백엔드는 HMAC 서명 bearer token을 발급하며 프론트엔드는 이후 API에 `Authorization: Bearer ...`를 전송합니다.
- 개발 mock 모드에서만 `/api/auth/me`가 `X-Employee-Id` fallback을 허용합니다. 운영 broker 모드는 callback query 또는 broker 내부 header가 필요합니다.
- 운영 전에는 `BROKER_URL`, `SERVICE_URL`, callback query 파라미터(`loginid`, `deptname`, `username`), `deptname` 조직 매핑, 방화벽/DNS를 실제 환경에서 검증해야 합니다.

## 사내 LLM용 SSO/메일 연동 가이드

성능이 낮은 내부 LLM이나 자동화 에이전트가 SSO 또는 메일 연동 코드를 수정해야 할 때는 [docs/sso-email-llm-guide.md](docs/sso-email-llm-guide.md)를 먼저 읽게 합니다. 이 문서는 mock/broker SSO 기준, `/api/auth/me` 사용 방식, 사내 `POST /send_mail` payload 경계를 코드 위치와 함께 정리합니다.
...
| `APP_BASE_URL` | `http://127.0.0.1:8000` | 승인 메일 상세 링크 생성 기준 URL |
| `SSO_MODE` | `mock` | `mock`, `broker` |
| `SSO_TOKEN_SECRET` | empty | mock bearer token 서명 secret |
| `SSO_TOKEN_EXPIRE_MINUTES` | `480` | bearer token 만료 시간 |
| `SSO_ADMIN_EMPLOYEE_IDS` | `admin001` | 콤마 구분 관리자 사번 |
| `SSO_BROKER_EMPLOYEE_HEADER` | `X-Broker-Employee-Id` | broker가 주입하는 사번 header |
| `SSO_BROKER_NAME_HEADER` | empty | broker가 주입하는 표시명 header |
| `SSO_BROKER_EMAIL_HEADER` | empty | broker가 주입하는 메일 header |
| `SSO_BROKER_DEPT_HEADER` | `deptname` | broker가 주입하는 소속명 header |
| `BROKER_URL` | empty | broker SSO 진입 URL. 예: `https://sso.example.com/svc0` |
| `SERVICE_URL` | empty | broker 인증 후 돌아올 서비스 URL. 예: `https://example1.com` |
| `MAIL_MODE` | `disabled` | `disabled`, `mail_api` |
| `MAIL_API_BASE_URL` | `mail.net` | 사내 메일 API base URL. `mail.net`이면 `https://mail.net/send_mail`로 호출 |
| `MAIL_API_SYSTEM_ID` | empty | 필요 시 `/send_mail` 요청 header `System-ID`로 전달 |
| `MAIL_API_TIMEOUT_SECONDS` | `10` | 메일 API 요청 timeout |

`SSO_MODE=broker`이면 `BROKER_URL`에서 인증한 뒤 `SERVICE_URL/?loginid=...&deptname=...&username=...` callback으로 돌아오는 흐름을 사용합니다. `MAIL_MODE=mail_api`이면 `MAIL_API_BASE_URL`이 필수입니다. 누락 시 앱 시작 단계에서 명확한 오류를 발생시킵니다.

`MAIL_MODE=mail_api`는 사내 메일 라우터 `POST /send_mail`로 JSON payload `{ "recipients": ["id@samsung.com"], "title": "제목", "content": "<html>" }`를 보냅니다. `docSecuType`, `contentType`, `recipientType` 조립은 메일 라우터가 담당합니다.
...
- `.env`, `.env.private-cloud`, DB 비밀번호, SSO token secret, 사내 메일 API 식별자는 커밋하지 않습니다.
- `.env.example`만 템플릿으로 커밋합니다.
- 운영 DB 계정은 앱 DB에 필요한 권한만 부여합니다.
- 운영에서는 `SSO_MODE=broker`를 사용하고, mock 모드의 `X-Employee-Id` fallback을 외부에 노출하지 않습니다. Broker callback URL의 `loginid`, `deptname`, `username` 값은 앱 세션 교환 직후 브라우저 주소창에서 제거됩니다.
...
`SSO_MODE=broker`인데 `BROKER_URL`, `SERVICE_URL`, callback query 파라미터 또는 broker 내부 header가 회사 환경과 다르면 로그인 후 현재 사용자 확인이 실패합니다. `MAIL_MODE=mail_api`인데 `MAIL_API_BASE_URL`이 비어 있으면 시작 중 오류가 납니다.
...
- [docs/sso-email-llm-guide.md](docs/sso-email-llm-guide.md): 사내 LLM용 SSO/메일 연동 구현 가이드
