...
def test_access_token_round_trips_user_payload():
    token = create_access_token(USER, secret="test-secret", now=datetime(2026, 4, 22, tzinfo=UTC))

    payload = verify_access_token(token, secret="test-secret", now=datetime(2026, 4, 22, tzinfo=UTC))
...
def test_access_token_rejects_tampering():
    ...
    with pytest.raises(HTTPException) as exc_info:
        verify_access_token(tampered, secret="test-secret", now=datetime(2026, 4, 22, tzinfo=UTC))
